...
const KnowledgeView = () => {
  ...
  const [activeTab, setActiveTab] = useState<'content' | 'upload' | 'search'>(
    'content'
  )
  ...
  if (!isEndpointActive) {
    return (
      <div className="text-muted flex flex-1 items-center justify-center">
        <p>Connect to AgentOS first</p>
      </div>
    )
  }

  return (
    <div className="bg-background/80 flex flex-1 flex-col overflow-hidden">
      {/* Tab bar */}
      <div className="border-primary/10 flex items-center gap-1 border-b p-2">
      ...
      {activeTab === 'content' && (
        <>
            ...
            {loading ? (
              ...
              <div className="flex flex-col gap-2">
                {content.map((item) => (
                  <div
                    key={item.id}
                    className="border-primary/10 bg-accent/50 flex items-center justify-between rounded-xl border p-4"
                  >
                    <div className="flex flex-1 flex-col gap-1">
                      <div className="flex items-center gap-2">
                        ...
                        {item.status && (
                          <span
                            className={`rounded-md px-2 py-0.5 text-xs ${statusColors[item.status]}`}
                          >
                            {item.status}
                          </span>
                        )}
          ...
          {totalPages > 1 && (
            <div className="border-primary/10 text-muted flex items-center justify-center gap-4 border-t p-3 text-xs">
              <Button
                variant="ghost"
                size="icon"
                disabled={page <= 1}
      ...
      {activeTab === 'upload' && (
        <div className="flex flex-1 flex-col gap-6 overflow-y-auto p-6">
          <div className="border-primary/15 bg-accent flex flex-col gap-3 rounded-xl border p-4">
            ...
            <input
              ...
              className="border-primary/15 bg-background h-9 rounded-xl border px-3 text-xs"
            />
            <div className="flex items-center gap-2">
              <input
                type="file"
                onChange={(e) => setUploadFile(e.target.files?.[0] ?? null)}
                className="text-muted file:border-primary/15 file:bg-background file:text-primary hover:file:bg-background-secondary flex-1 text-xs file:mr-3 file:rounded-lg file:border file:px-3 file:py-1.5 file:text-xs"
              />
            </div>
            {uploadFile && (
              <p className="text-muted text-xs">
                Selected: {uploadFile.name} (
                {formatSize(String(uploadFile.size))})
              </p>
            )}
            <Button
              size="sm"
              className="bg-primary text-primaryAccent hover:bg-primary/90"
          ...
          <div className="border-primary/15 bg-accent flex flex-col gap-3 rounded-xl border p-4">
            ...
            <div className="flex gap-2">
              <input
                ...
                className="border-primary/15 bg-background h-9 flex-1 rounded-xl border px-3 text-xs"
              />
              <Button
                size="sm"
            ...
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-2">
                ...
                <select
                  ...
                  className="border-primary/15 bg-background h-8 rounded-lg border px-2 text-xs"
                >
              ...
              <div className="flex items-center gap-2">
                <label className="text-muted text-xs">Max results:</label>
                <input
                  type="range"
                  min={1}
                  max={20}
                  ...
                  className="w-24"
                />
                <span className="text-muted text-xs">{maxResults}</span>
              </div>
            </div>
          </div>
            ...
            {searchResults.length === 0 ? (
              ...
              <div className="flex flex-col gap-2">
                {searchResults.map((result) => (
                  <div
                    key={result.id}
                    className="border-primary/10 bg-accent/50 rounded-xl border p-4"
                  >
                    <div className="flex items-center justify-between">
                      ...
                      {result.reranking_score !== null && (
                        <span className="text-muted text-xs">
                          Score: {(result.reranking_score * 100).toFixed(1)}%
                        </span>
                      )}
                    </div>
                    <p className="text-muted mt-2 line-clamp-4 text-sm">
                      {result.content}
                    </p>
                  </div>
                ))}
