...
const formatDate = (date: string | number | null | undefined) => {
  if (!date) return '—'
  return dayjs(typeof date === 'number' ? date * 1000 : date).format(
    'DD MMM YYYY, HH:mm'
  )
}
...
const SessionDetailPanel = ({
  ...
  const [detail, setDetail] = useState<
    AgentSessionDetail | TeamSessionDetail | WorkflowSessionDetail | null
  >(null)
  ...
  const fetchDetail = useCallback(async () => {
    ...
    try {
      ...
      const data = await getSessionDetailAPI(
        base,
        sessionId,
        type,
        undefined,
  ...
  const chatHistory =
    (detail as AgentSessionDetail).chat_history ||
    (detail as TeamSessionDetail).chat_history ||
    []
  const totalTokens = (detail as AgentSessionDetail).total_tokens

  return (
    <div className="flex flex-1 flex-col overflow-hidden">
      <div className="border-primary/10 flex items-center justify-between border-b p-4">
          ...
          {isEditing ? (
            <div className="flex items-center gap-2">
              <input
                ...
                className="border-primary/20 bg-accent rounded-lg border px-3 py-1.5 text-sm"
                autoFocus
              />
      ...
      <div className="border-primary/10 text-muted flex gap-4 border-b px-4 py-2 text-xs">
        <span>
        ...
        {totalTokens !== undefined && totalTokens !== null && (
          <span>
            Tokens: <span className="text-foreground">{totalTokens}</span>
          </span>
        )}
      </div>
            ...
            {chatHistory.map(
              (
                ...
                return (
                  ...
                  >
                    <div
                      className={`max-w-[80%] rounded-xl p-3 text-sm ${isUser ? 'bg-primary/10 text-foreground' : 'bg-accent text-muted'}`}
                    >
...
const SessionsView = () => {
  ...
  const fetchSessions = useCallback(async () => {
    ...
    try {
      const base = constructEndpointUrl(selectedEndpoint)
      const result = await getAllSessionsPaginatedAPI(
        base,
        {
          limit: 20,
          page,
  ...
  return (
    <div className="bg-background/80 flex flex-1 flex-col overflow-hidden">
      <div className="border-primary/10 flex items-center justify-between border-b p-4">
        <h2 className="text-primary text-sm font-medium uppercase">
          Sessions ({totalCount})
        </h2>
        <div className="flex items-center gap-2">
          <div className="relative">
            ...
            <input
              ...
              className="border-primary/15 bg-accent h-9 w-64 rounded-xl border pl-9 pr-3 text-xs"
            />
          </div>
        </div>
      </div>
          ...
          <div className="flex flex-col gap-1 p-4">
            {sessions.map((session) => (
              <div
                ...
                className="border-primary/10 bg-accent/50 hover:border-primary/20 hover:bg-accent flex items-center justify-between rounded-xl border p-4 transition-colors"
              >
                <div
                  ...
                  onClick={() =>
                    setSelectedSession({
                      ...
                      type: session.session_type || 'agent'
                    })
                  }
                >
